...
STATUS channel_info_create(PChannelInfo pOrigChannelInfo, PChannelInfo* ppChannelInfo)
{
    ...
    if (pOrigChannelInfo->pControlPlaneUrl != NULL && *pOrigChannelInfo->pControlPlaneUrl != '\0') {
        STRNCPY(pCurPtr, pOrigChannelInfo->pControlPlaneUrl, cpUrlLen);
    } else {
        // Create a fully qualified URI
        SNPRINTF(pCurPtr, cpUrlLen, "%s%s.%s%s", CONTROL_PLANE_URI_PREFIX, KINESIS_VIDEO_SERVICE_NAME, pChannelInfo->pRegion,
