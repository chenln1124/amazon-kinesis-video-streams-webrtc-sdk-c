...
STATUS signaling_send(PSignalingClient pSignalingClient, PSignalingMessage pSignalingMessage)
{
    ...
    // DLOGD("%s: %d, pSignalingMessage->payload: %s.", __func__, __LINE__, pSignalingMessage->payload);
...
STATUS signaling_joinSession(PSignalingClient pSignalingClient)
{
    ...
    // Store the signaling state in case we error/timeout so we can re-set it on exit
    state = signaling_fsm_getCurrentState(pSignalingClient);
    CHK(signaling_fsm_step(pSignalingClient, SIGNALING_GET_CURRENT_TIME(pSignalingClient) + SIGNALING_JOIN_SESSION_STATE_TIMEOUT,
                           SIGNALING_STATE_JOIN_SESSION) == STATUS_SUCCESS, STATUS_SIGNALING_FSM_STEP_FAILED);
...
STATUS signaling_channel_joinStorageSession(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        if (ATOMIC_LOAD_BOOL(&pSignalingClient->connected)) {
            ...
            ATOMIC_STORE(&pSignalingClient->apiCallStatus, (SIZE_T) HTTP_STATUS_OK); //!< TBD
        }
    }
